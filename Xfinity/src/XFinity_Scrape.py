...
def schedule():
    ...
    """
    thisyear = int(date.today().year)+1
    ...
    race_date = []
    for year in years:
        url = requests.get("https://www.racing-reference.info/season-stats/" + year + "/B/").text
        sleep(randint(2,4))
        ...
        for i in range(len(my_table)):
            dates = datetime.strptime(my_table[i].find('div', class_ = "date B").text.strip(), "%m/%d/%y").strftime("%m/%d/%y")
            corrected = datetime.strptime(dates, "%m/%d/%y")
            corrected = datetime.date(corrected)
            if corrected < today:
                race_num.append(int(my_table[i].div.a.text))
                track.append(my_table[i].find('div', class_ = 'track B').text.rstrip())
            ...
            if corrected == today:
                ...
                year_list.append(int(year))
                race_date.append(corrected)
                link = my_table[i].find('div', class_ = 'track B').a.get('href')
    ...
    loop.to_csv('../data/raw/XFinity_loop.csv')
    schedules.to_csv('../data/raw/XFinity_schedule.csv')
    qualifying.to_csv('../data/raw/XFinity_qual.csv')
...
def entrylist(df):
    ...
    for i in range(len(df)):
        ...
        url = requests.get("https://www.racing-reference.info/entrylist/" + year + '-' + race_num + "/B").text
        sleep(randint(2, 4))
        soup = BeautifulSoup(url, 'html.parser')
    ...
    print(career_stats)
    entry_list.to_csv('../data/raw/XFinity_entry.csv')
    career_stats.to_csv('../data/raw/XFinity_driver_career.csv')
...
def driver_career(driver, entry_list):
    ...
    driver2 = []
    for names in driver:
        if names == 'Morgan Shepherd':
            pass
        else:
            link = entry_list.loc[entry_list['driver'] == names]
            link = np.array(link['driver_link'])
            link = link[0].split('/')[4]
            url = requests.get('https://www.racing-reference.info/rquery?id=' + link +'&trk=t0&series=B').text
            sleep(randint(2, 5))
            soup = BeautifulSoup(url, 'html.parser')
            my_table = soup.find_all('tr', {'class': "odd"})
            my_table2 = soup.find_all('tr', {'class': "even"})
            for i in range(len(my_table)):
                ...
                start.append(a[3].text)
                finish.append(a[4].text)
    ...
    df[['start_pos', 'finish_pos']] = df[['start_pos', 'finish_pos']].astype(int)
    return df
...
def getqual(data):
    ...
    for years in np.unique(year):
        ...
        for race in np.unique(races):
            race_num = race
            url = requests.get('https://www.racing-reference.info/getqualify/' + year + '-' + race_num + '/B').text
            sleep(randint(2, 4))
            soup = BeautifulSoup(url, 'html.parser')
            my_table = soup.find_all('tr', {'class': "odd"})
            my_table2 = soup.find_all('tr', {'class': "even"})
    ...
    for years in np.unique(year):
        ...
        for race in np.unique(races):
            ...
            for j in range(1, 6):
                j = str(j)
                url = requests.get(
                    'https://www.racing-reference.info/getpractice/' + year + '-' + race_num + '/B/' + j).text
                sleep(randint(2, 4))
                soup = BeautifulSoup(url, 'html.parser')
                my_table = soup.find_all('tr', {'class': "odd"})
                my_table2 = soup.find_all('tr', {'class': "even"})
...
def getloops(data):
    ...
    for years in np.unique(year):
        ...
        for race in np.unique(races):
            race_num = race
            url = requests.get('https://www.racing-reference.info/loopdata/' + year + '-' + race_num + '/B').text
            sleep(randint(2, 4))
            soup = BeautifulSoup(url, 'html.parser')
            my_table = soup.find_all('tr', {'class': "odd"})
            my_table2 = soup.find_all('tr', {'class': "even"})
