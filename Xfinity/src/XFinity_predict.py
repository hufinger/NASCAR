...
def tier(df, val1, val2):
    ...
    med = .75
    low = .5
    if df[val1] >= high or df[val2] >= high:
        return '1.5x'
...
def predict_h2h(h2h):
    ...
    with open('../prediction_artifacts/H2HX_1.0.0', 'rb') as file:
    ...
    h2h_result.to_csv('../data/predictions/h2h_predictions.csv')
...
def predict_placements(next):
    ...
    with open('../prediction_artifacts/Top3ModX_1.0.0', 'rb') as file:
        mod3 = pickle.load(file)
    with open('../prediction_artifacts/Top5ModX_1.0.0', 'rb') as file:
        mod5 = pickle.load(file)
    with open('../prediction_artifacts/Top10ModX_1.0.0', 'rb') as file:
    ...
    print(finish_pos)
    finish_pos.to_csv('../data/predictions/FinishPos.csv')
...
def predict_dfs(dfs_next):
    ...
    with open('../prediction_artifacts/DFSX_1.0.0.pkl', 'rb') as file:
    ...
    dfs_results.to_csv('../data/predictions/dfs_predictions.csv')


if __name__ == '__main__':
    h2h = pd.read_csv('../data/next_race/H2H_next_XFinity.csv', index_col=[0])
    next = pd.read_csv('../data/next_race/next_race_XFinity.csv', index_col=[0])
    dfs_next = pd.read_csv('../data/next_race/DFS_next_XFinity.csv', index_col=[0])
